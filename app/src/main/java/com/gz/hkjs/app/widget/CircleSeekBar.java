...
/**
 * Created by husong on 2017/5/10.
 */
...
public class CircleSeekBar extends View{
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            ...
            mVelocity = type.getFloat(R.styleable.CircleSeekBar_velocity,
                    VELOCITY_DEFAULT);
            mTextSize = type.getDimension(R.styleable.CircleSeekBar_c_textSize,
                    Util.dip2px(mContext, TEXT_SIZE_DEFAULT));
            mTextColor = type.getColor(R.styleable.CircleSeekBar_c_textColor,
                    TEXT_COLOR_DEFAULT);
            mProgressStrokeWidth = type.getDimension(
                    R.styleable.CircleSeekBar_progressWidth,
                    Util.dip2px(mContext, PROGRESS_WIDTH_DEFAULT));
            ...
            mSProgressStrokeWidth = type.getDimension(
                    ...
                    Util.dip2px(mContext, S_PROGRESS_WIDTH_DEFAULT));
            ...
            mCapRound = type.getBoolean(R.styleable.CircleSeekBar_capRound,
                    true);
            float progress = type.getFloat(R.styleable.CircleSeekBar_c_progress,
                    0);
        ...
        mBgmPaint = new Paint(mPaint);
        mBgmPaint.setColor(Color.parseColor("#70000000"));
        mBgmPaint.setStyle(Style.FILL);
        if (mCapRound) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            int desired = (int) (getPaddingLeft()
                    + Util.dip2px(mContext, MIN_WIDTH) + getPaddingRight());
            width = desired;
        }
        if (heightMode == MeasureSpec.EXACTLY) {
            ...
            int desired = (int) (getPaddingTop()
                    + Util.dip2px(mContext, MIN_HEIGHT) + getPaddingBottom());
            height = desired;
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawArc(mSProgressRect, 0, 360f, false, mBgmPaint);
...
//            String text = formatProgress(ratio * mMaxProgress);
    ...
    public void setStepText(int progress){
        mText = progress +"/"+(int) mMaxProgress;
    }

    public void setDaojishiText(int progress){
        mText = progress+"";
    }
